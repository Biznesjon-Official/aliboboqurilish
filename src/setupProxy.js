...
module.exports = function(app) {
    ...
    createProxyMiddleware({
      ...
      onError: (err, req, res) => {
        ...
        checkBackendHealth().then(isHealthy => {
          if (!isHealthy) {
            console.log('❌ Backend server is not responding on port 3000');
        ...
        if (!res.headersSent) {
          res.status(504).json({ 
            ...
            message: 'Backend server timeout - check if server is running on port 3000',
  ...
  app.use(
    ...
    createProxyMiddleware({
      ...
      onProxyReq: (proxyReq, req, res) => {
        if (process.env.REACT_APP_DEBUG_MODE === 'true') {
          console.log(`[PROXY] Upload Request: ${req.method} ${req.url} -> http://localhost:3000${req.url}`);
        }
      }
    })
  );
  ...
  app.use(
    ...
    createProxyMiddleware({
      ...
      onError: (err, req, res) => {
        ...
        console.log('🔍 Check if backend Socket.IO server is running on port 3000');
        if (!res.headersSent) {
          res.status(504).end();
        }
      },
      onProxyReq: (proxyReq, req, res) => {
        console.log(`[SOCKET.IO] 🔌 ${req.method} ${req.url} -> http://localhost:3000${req.url}`);
      }
    })
  );
};